...
{
    PtrList<SolidThermoType> speciesData(solidSpecies_.size());
...
template<class SolidThermoType, class GasThermoType>
typename Foam::multiComponentGSMixture<SolidThermoType, GasThermoType>::speciesCompositionTable
Foam::multiComponentGSMixture<SolidThermoType, GasThermoType>::readSolidSpeciesComposition
(
    const dictionary& thermoDict,
...
) const
{
    ...
    {
        if (thermoDict.subDict(solidSpecies[si]).isDict("elements"))
        {
            dictionary currentElements
            (
                thermoDict.subDict(solidSpecies[si]).subDict("elements")
            );
            ...
            // Add current specie composition to the hash table
            speciesCompositionTable::iterator solidSpecieCompositionIter
            (
                solidSpeciesComposition_.find(solidSpecies[si])
            );

            if (solidSpecieCompositionIter != solidSpeciesComposition_.end())
            {
                solidSpeciesComposition_.erase(solidSpecieCompositionIter);
            }

            solidSpeciesComposition_.insert(solidSpecies[si], currentComposition);
        }
    }
...
:
    ...
    solidSpeciesComposition_(readSolidSpeciesComposition(thermoDict.subDict("solid"), this->solidSpecies())),
...
:
    ...
    speciesData_(readSpeciesData(thermoDict.subDict("gas"))),
    speciesComposition_(readSpeciesComposition(thermoDict.subDict("gas"), species())),
    solidSpeciesComposition_(readSolidSpeciesComposition(thermoDict.subDict("solid"), solidSpecies())),
    mixture_("mixture", solidSpeciesData_[0]),
    mixtureVol_("volMixture", solidSpeciesData_[0]),
    gasMixture_("gasMixture", speciesData_[0]),
    gasMixtureVol_("gasVolMixture", speciesData_[0])
