...
) const
{
    ...
    forAll(this->lhs(), s)
    {
        label si = reduced ? c2s[this->lhs()[s].index + nGases_] : (this->lhs()[s].index + nGases_);
        scalar sl = this->lhs()[s].stoichCoeff;
        dcdt[si] -= sl*omegai; // [kmol/m2]
    }
    forAll(this->glhs(), g)
    {
        label gi = reduced ? c2s[this->glhs()[g].index] : this->glhs()[g].index;
        scalar gl = this->glhs()[g].stoichCoeff;
        dcdt[gi] -= gl*omegai; // [kmol/m2]
    }
    forAll(this->rhs(), s)
    {
        label si = reduced ? c2s[this->rhs()[s].index + nGases_] : (this->rhs()[s].index + nGases_);
        scalar sr = this->rhs()[s].stoichCoeff;
        dcdt[si] += sr*omegai; // [kmol/m2]
    }
    forAll(this->grhs(), g)
    {
        label gi = reduced ? c2s[this->grhs()[g].index] : this->grhs()[g].index;
        scalar gr = this->grhs()[g].stoichCoeff;
        dcdt[gi] += gr*omegai; // [kmol/m2]
    }
}
...
) const
{
    ...
    forAll(this->lhs(), j)
    {
        const label sj = reduced ? c2s[this->lhs()[j].index + nGases_] : (this->lhs()[j].index + nGases_);
        scalar kf = kfwd;
        forAll(this->lhs(), i)
        {
        ...
        forAll(this->lhs(), i)
        {
            const label si = reduced ? c2s[this->lhs()[i].index + nGases_] : (this->lhs()[i].index + nGases_);
            const scalar sl = this->lhs()[i].stoichCoeff;
            J(si, sj) -= sl*kf;
        }
        forAll(this->glhs(), i)
        {
            const label si = reduced ? c2s[this->glhs()[i].index] : this->glhs()[i].index;
            const scalar sl = this->glhs()[i].stoichCoeff;
            J(si, sj) -= sl*kf;
        }
        forAll(this->rhs(), i)
        {
            const label si = reduced ? c2s[this->rhs()[i].index + nGases_] : (this->rhs()[i].index + nGases_);
            const scalar sr = this->rhs()[i].stoichCoeff;
            J(si, sj) += sr*kf;
        }
        forAll(this->grhs(), i)
        {
            const label si = reduced ? c2s[this->grhs()[i].index] : this->grhs()[i].index;
    ...
    forAll(this->glhs(), j)
    {
        const label sj = reduced ? c2s[this->glhs()[j].index] : this->glhs()[j].index;
        scalar kf = kfwd;
        forAll(this->glhs(), i)
        {
            const label si = this->glhs()[i].index;
    ...
    forAll(this->rhs(), j)
    {
        const label sj = reduced ? c2s[this->rhs()[j].index + nGases_] : (this->rhs()[j].index + nGases_);
        scalar kr = kbwd;
        forAll(this->rhs(), i)
        {
    ...
    forAll(this->grhs(), j)
    {
        const label sj = reduced ? c2s[this->grhs()[j].index] : this->grhs()[j].index;
        scalar kr = kbwd;
        forAll(this->grhs(), i)
        {
            const label si = this->grhs()[i].index;
    ...
    if (notNull(covdep))
    {
        ...
        forAll(covdep, j)
        {
            label sj = reduced ? c2s[covdep[j].first()] : covdep[j].first();
            if (sj != -1)
            {
                forAll(this->lhs(), i)
                {
