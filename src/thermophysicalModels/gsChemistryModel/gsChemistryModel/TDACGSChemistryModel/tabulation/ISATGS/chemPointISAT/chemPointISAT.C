...
Foam::scalar Foam::chemPointISAT<CompType, GThermoType, SThermoType>::tolerance_;

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::qrDecompose
(
    const label nCols,
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::qrUpdate
(
    scalarSquareMatrix& R,
    const label n,
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::rotate
(
    scalarSquareMatrix& R,
    const label i,
    const scalar a,
...
template<class CompType, class GThermoType, class SThermoType>
Foam::chemPointISAT<CompType, GThermoType, SThermoType>::chemPointISAT
(
    ...
    binaryNode<CompType, GThermoType, SThermoType>* node
)
:
    ...
    (
        completeSpaceSize - (4 + (variableTimeStep() == 1 ? 1 : 0))
    )
{
    ...
    idArea_ = completeSpaceSize - nAdditionalEqns_+ 2;
    idPore_ = completeSpaceSize - nAdditionalEqns_ + 3;
...
Foam::chemPointISAT<CompType, GThermoType, SThermoType>::chemPointISAT
(
    Foam::chemPointISAT<CompType, GThermoType, SThermoType>& p
)
:
    ...
    completeToSimplifiedIndex_(p.completeToSimplifiedIndex())
{
    ...
    {
        nAdditionalEqns_ = 5;
        idT_ = completeSpaceSize() - 5;
        idp_ = completeSpaceSize() - 4;
        idArea_ = completeSpaceSize() - 3;
        idPore_ = completeSpaceSize() - 2;
        iddeltaT_ = completeSpaceSize() - 1;
    }
    else
    {
      nAdditionalEqns_ = 4;
      idT_ = completeSpaceSize() - 4;
      idp_ = completeSpaceSize() - 3;
      idArea_ = completeSpaceSize() - 2;
      idPore_ = completeSpaceSize() - 1;
      iddeltaT_ = completeSpaceSize(); // will not be used
    }
}
...
template<class CompType, class GThermoType, class SThermoType>
bool Foam::chemPointISAT<CompType, GThermoType, SThermoType>::inEOA(const scalarField& phiq)
{
    ...
    for (label i=0; i<completeSpaceSize()-nAdditionalEqns_; i++)
    {
        ...
        )
        {
            ...
            temp += LT_(si, dim+2)*dphi[idArea_];
            temp += LT_(si, dim+3)*dphi[idPore_];
            if (variableTimeStep())
            {
                temp += LT_(si, dim+4)*dphi[iddeltaT_];
            }
    ...
    if (variableTimeStep())
    {
        epsTemp +=
            ...
            (
               ...
               +LT_(dim, dim+2)*dphi[idArea_]
               +LT_(dim, dim+3)*dphi[idPore_]
               +LT_(dim, dim+4)*dphi[iddeltaT_]
            );
    }
    else
    {
    ...
    if (variableTimeStep())
    {
        epsTemp +=
            ...
            (
               ...
               +LT_(dim+1, dim+2)*dphi[idArea_]
               +LT_(dim+1, dim+3)*dphi[idPore_]
               +LT_(dim+1, dim+4)*dphi[iddeltaT_]
            );
    }
    else
    {
    ...
    if (variableTimeStep())
    {
        epsTemp +=
            sqr
            (
               +LT_(dim+2, dim+2)*dphi[idArea_]
               +LT_(dim+2, dim+3)*dphi[idPore_]
               +LT_(dim+2, dim+4)*dphi[iddeltaT_]
            );
    }
    else
    {
    ...
    if (variableTimeStep())
    {
        epsTemp +=
            sqr
            (
               +LT_(dim+3, dim+3)*dphi[idPore_]
               +LT_(dim+3, dim+4)*dphi[iddeltaT_]
            );
    }
    else
    {
    ...
    if (variableTimeStep())
    {
        epsTemp += sqr(LT_[dim+4][dim+4]*dphi[iddeltaT_]);
    }
    ...
    if (printProportion_)
    {
        ...
        propEps[idArea_] =
            sqr(LT_(dim+2, dim+2)*dphi[idArea_]);

        propEps[idPore_] =
            sqr(LT_(dim+3, dim+3)*dphi[idPore_]);

        if (variableTimeStep())
        {
            propEps[iddeltaT_] =
                sqr(LT_[dim+4][dim+4]*dphi[iddeltaT_]);
        }
    ...
    if (sqrt(epsTemp) > 1 + tolerance_)
    {
        if (printProportion_)
        {
            ...
            if (maxIndex >= completeSpaceSize() - nAdditionalEqns_)
            {
                ...
                else if (maxIndex == idArea_)
                {
                    propName = "SurfaceArea";
                }
                else if (maxIndex == idPore_)
                {
                    propName = "Porosity";
                }
            ...
            else if (maxIndex >= chemistry_.Y().size())
            {
                propName = chemistry_.Ys()[maxIndex - chemistry_.Y().size()].member();
            }
            else
            {
...
template<class CompType, class GThermoType, class SThermoType>
bool Foam::chemPointISAT<CompType, GThermoType, SThermoType>::checkSolution
(
    ...
    for (label i=0; i<completeSpaceSize()-nAdditionalEqns_; i++)
    {
        ...
        if (isMechRedActive)
        {
            ...
            if (si != -1)
            {
                ...
                dRl += Avar(si, nActiveSpecies_+2)*dphi[idArea_];
                dRl += Avar(si, nActiveSpecies_+3)*dphi[idPore_];
                if (variableTimeStep())
                {
                    dRl += Avar(si, nActiveSpecies_+4)*dphi[iddeltaT_];
                }
            }
            else
            {
...
template<class CompType, class GThermoType, class SThermoType>
bool Foam::chemPointISAT<CompType, GThermoType, SThermoType>::grow(const scalarField& phiq)
{
    scalarField dphi(phiq - phi());
    ...
    if (isMechRedActive)
    {
        ...
        if (nActiveSpecies_ > initNActiveSpecies)
        {
            ...
            LT_(nActiveSpecies_+3, nActiveSpecies_+3)=
                LTvar(initNActiveSpecies+3, initNActiveSpecies+3);
            A_(nActiveSpecies_+3, nActiveSpecies_+3)=
                Avar(initNActiveSpecies+3, initNActiveSpecies+3);

            if (variableTimeStep())
            {
                LT_(nActiveSpecies_+4, nActiveSpecies_+4)=
                    LTvar(initNActiveSpecies+2, initNActiveSpecies+4);
                A_(nActiveSpecies_+4, nActiveSpecies_+4)=
                    Avar(initNActiveSpecies+4, initNActiveSpecies+4);
            }
    ...
    for (label i=0; i<dim; i++)
    {
        ...
        phiTilde[i] += LT_(i, dim-nAdditionalEqns_+2)*dphi[idArea_];
        phiTilde[i] += LT_(i, dim-nAdditionalEqns_+3)*dphi[idPore_];

        if (variableTimeStep())
        {
            phiTilde[i] += LT_(i, dim-nAdditionalEqns_ + 4)*dphi[iddeltaT_];
        }
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::increaseNumRetrieve()
{
    this->numRetrieve_++;
}
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::resetNumRetrieve()
{
    this->numRetrieve_ = 0;
}
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemPointISAT<CompType, GThermoType, SThermoType>::increaseNLifeTime()
{
    this->nLifeTime_++;
}
...
template<class CompType, class GThermoType, class SThermoType>
Foam::label Foam::chemPointISAT<CompType, GThermoType, SThermoType>::
...
)
{
    ...
    else if (i == nActiveSpecies_ + 3)
    {
        return completeSpaceSize_-nAdditionalEqns_ + 3;
    }
    else if (variableTimeStep() && (i == nActiveSpecies_ + 4))
    {
        return completeSpaceSize_-nAdditionalEqns_ + 4;
    }
    else
    {
        return -1;
