...
Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::ISATGS
(
    const dictionary& chemistryProperties,
    TDACGSChemistryModel<CompType, GThermoType, SThermoType>& chemistry
)
:
    chemistryGSTabulationMethod<CompType, GThermoType, SThermoType>
    (
        chemistryProperties,
        chemistry
    ),
    chemisTree_(chemistry, this->coeffsDict_),
    scaleFactor_(chemistry.nEqns() + ((this->variableTimeStep()) ? 3 : 2), 1),
    ...
    cleaningRequired_(false)
{
    if (this->active_)
    {
        ...
        label Yssize = this->chemistry_.Ys().size();
        scalar otherSolidScaleFactor = scaleDict.lookup<scalar>("otherSolidSpecies");
        for (label i=0; i<Yssize; i++)
        {
            if (!scaleDict.found(this->chemistry_.Ys()[i].member()))
            {
                scaleFactor_[i+Ysize] = otherSolidScaleFactor;
            }
            else
            {
                scaleFactor_[i+Ysize] =
                    scaleDict.lookup<scalar>(this->chemistry_.Ys()[i].member());
            }
        }
        scaleFactor_[Ysize + Yssize] = scaleDict.lookup<scalar>("Temperature");
        scaleFactor_[Ysize + Yssize + 1] = scaleDict.lookup<scalar>("Pressure");
        scaleFactor_[Ysize + Yssize + 2] = scaleDict.lookup<scalar>("SurfaceArea");
        scaleFactor_[Ysize + Yssize + 3] = scaleDict.lookup<scalar>("Porosity");
        if (this->variableTimeStep())
        {
            scaleFactor_[Ysize + Yssize + 4] = scaleDict.lookup<scalar>("deltaT");
        }
    }
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::addToMRU
(
    chemPointISAT<CompType, GThermoType, SThermoType>* phi0
)
{
    if (maxMRUSize_ > 0 && MRURetrieve_)
    {
        ...
        bool isInList = false;
        typename SLList <chemPointISAT<CompType, GThermoType, SThermoType>*>::iterator iter =
            MRUList_.begin();
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::calcNewC
(
    chemPointISAT<CompType, GThermoType, SThermoType>* phi0,
    ...
    for (label i=0; i<nEqns-nAdditionalEqns_; i++)
    {
        if (mechRedActive)
        {
            ...
            if (si != -1)
            {
                ...
                Rphiq[i] +=
                    gradientsMatrix(si, phi0->nActiveSpecies())*dphi[nEqns - 4];
                Rphiq[i] +=
                ...
                Rphiq[i] +=
                    ...
                    *dphi[nEqns - 2];
                Rphiq[i] +=
                    gradientsMatrix(si, phi0->nActiveSpecies() + 3)
                    *dphi[nEqns - 1];

                if (this->variableTimeStep())
                {
                    Rphiq[i] +=
                        gradientsMatrix(si, phi0->nActiveSpecies() + 4)
                       *dphi[nEqns];
                }
...
template<class CompType, class GThermoType, class SThermoType>
bool Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::grow
(
    chemPointISAT<CompType, GThermoType, SThermoType>* phi0,
    const scalarField& phiq,
...
template<class CompType, class GThermoType, class SThermoType>
bool
Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::cleanAndBalance()
{
    ...
    chemPointISAT<CompType, GThermoType, SThermoType>* x = chemisTree_.treeMin();
    while(x != nullptr)
    {
        chemPointISAT<CompType, GThermoType, SThermoType>* xtmp =
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::computeA
(
...
{
    bool mechRedActive = this->chemistry_.mechRed()->active();
    label gspeciesNumber = this->chemistry_.Y().size();
    label gspeciesNumberActive = this->chemistry_.nGasSpecie();
    label sspeciesNumber = this->chemistry_.Ys().size();
    label sspeciesNumberActive = this->chemistry_.nSolidSpecie();
    label nspeciesNumberActive = sspeciesNumberActive + gspeciesNumberActive;

    scalarField Rcq(this->chemistry_.nEqns() + nAdditionalEqns_ - 2);

    for (label i=0; i<gspeciesNumberActive; i++)
    {
        label s2c = i;
        if (mechRedActive)
        {
        ...
        Rcq[i] = rhoi*Rphiq[s2c]/this->chemistry_.gasSpecieThermo()[s2c].W();
    }

    for (label i=0; i<sspeciesNumberActive; i++)
    {
        label s2c = i + gspeciesNumberActive;
        if (mechRedActive)
        {
            s2c = this->chemistry_.simplifiedToCompleteIndex()[s2c];
            Rcq[s2c] = this->chemistry_.solidSpecieThermo()[s2c - gspeciesNumber].sden()*Rphiq[s2c]/this->chemistry_.solidSpecieThermo()[s2c - gspeciesNumber].size();
        }
        else
        {
            Rcq[s2c] = this->chemistry_.solidSpecieThermo()[i].sden()*Rphiq[s2c]/this->chemistry_.solidSpecieThermo()[i].size();
        }
    }

    Rcq[nspeciesNumberActive] = Rphiq[Rphiq.size() - nAdditionalEqns_];
    Rcq[nspeciesNumberActive + 1] = Rphiq[Rphiq.size() - nAdditionalEqns_ + 1];
    Rcq[nspeciesNumberActive + 2] = Rphiq[Rphiq.size() - nAdditionalEqns_ + 2];
    Rcq[nspeciesNumberActive + 3] = Rphiq[Rphiq.size() - nAdditionalEqns_ + 3];
    if (this->variableTimeStep())
    {
        Rcq[nspeciesNumberActive + 4] = Rphiq[Rphiq.size() - nAdditionalEqns_ + 4];
    }
    ...
    // where C(psi0,t0) = I
    scalarField dcdt(nspeciesNumberActive + 2, Zero);
    ...
    for (label i=0; i<gspeciesNumberActive; i++)
    {
        ...
        for (label j=0; j<gspeciesNumberActive; j++)
        {
            label sj = j;
            if (mechRedActive)
            {
            ...
            A(i, j) *=
              -dt*this->chemistry_.gasSpecieThermo()[si].W()
               /this->chemistry_.gasSpecieThermo()[sj].W();
        }

        for (label j=0; j<sspeciesNumberActive; j++)
        {
            label sj = j + gspeciesNumberActive;
            if (mechRedActive)
            {
                sj = this->chemistry_.simplifiedToCompleteIndex()[sj];
                A(i, j + gspeciesNumberActive) *=
                    -dt*this->chemistry_.gasSpecieThermo()[si].W()/rhoi
                    *this->chemistry_.solidSpecieThermo()[sj - gspeciesNumber].sden()/this->chemistry_.solidSpecieThermo()[sj - gspeciesNumber].size();
            }
            else
            {
                A(si, sj) *=
                    -dt*this->chemistry_.gasSpecieThermo()[si].W()/rhoi
                    *this->chemistry_.solidSpecieThermo()[j].sden()/this->chemistry_.solidSpecieThermo()[j].size();
            }
        }

        A(i, i) += 1;
        // Columns for pressure and temperature
        A(i, nspeciesNumberActive) *=
            -dt*this->chemistry_.gasSpecieThermo()[si].W()/rhoi;
        A(i, nspeciesNumberActive + 1) *=
            -dt*this->chemistry_.gasSpecieThermo()[si].W()/rhoi;
    }

    for (label i=0; i<sspeciesNumberActive; i++)
    {
        label si = i + gspeciesNumberActive;
        if (mechRedActive)
        {
            si = this->chemistry_.simplifiedToCompleteIndex()[si];
        }
        ...
        for (label j=0; j<gspeciesNumberActive; j++)
        {
            ...
            {
                sj = this->chemistry_.simplifiedToCompleteIndex()[sj];
                A(i + gspeciesNumberActive, j) *=
                  -dt/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size()
                   /this->chemistry_.gasSpecieThermo()[sj].W()*rhoi;
            }
            else
            {
                A(si , sj) *=
                  -dt/this->chemistry_.solidSpecieThermo()[i].sden()*this->chemistry_.solidSpecieThermo()[i].size()
                   /this->chemistry_.gasSpecieThermo()[sj].W()*rhoi;
            }
        ...
        for (label j=0; j<sspeciesNumberActive; j++)
        {
            ...
            {
                sj = this->chemistry_.simplifiedToCompleteIndex()[sj];
                A(i + gspeciesNumberActive, j + gspeciesNumberActive) *=
                  -dt/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size()
                   *this->chemistry_.solidSpecieThermo()[sj - gspeciesNumber].sden()/this->chemistry_.solidSpecieThermo()[sj - gspeciesNumber].size();
            }
            else
        ...
        if(mechRedActive)
        {
            A(si , nspeciesNumberActive) *=
                -dt/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size();
            A(si , nspeciesNumberActive + 1) *=
                -dt/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size();
        }
        else
        {
    ...
    for (label i=0; i<gspeciesNumberActive; i++)
    {
        ...
        A(nspeciesNumberActive, i) *=
            -dt*rhoi/this->chemistry_.gasSpecieThermo()[si].W();
        A(nspeciesNumberActive + 1, i) *=
            -dt*rhoi/this->chemistry_.gasSpecieThermo()[si].W();
    }
    ...
    for (label i=0; i<sspeciesNumberActive; i++)
    {
        ...
        {
            si = this->chemistry_.simplifiedToCompleteIndex()[si];
            A(nspeciesNumberActive, i + gspeciesNumberActive) *=
                -dt*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size();
            A(nspeciesNumberActive + 1, i + gspeciesNumberActive) *=
                -dt*this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].sden()/this->chemistry_.solidSpecieThermo()[si - gspeciesNumber].size();
        }
        else
        {
            A(nspeciesNumberActive, si) *=
                -dt*this->chemistry_.solidSpecieThermo()[i].sden()/this->chemistry_.solidSpecieThermo()[i].size();
            A(nspeciesNumberActive + 1, si) *=
                -dt*this->chemistry_.solidSpecieThermo()[i].sden()/this->chemistry_.solidSpecieThermo()[i].size();
        }
    }

    A(nspeciesNumberActive, nspeciesNumberActive) = -dt*A(nspeciesNumberActive, nspeciesNumberActive) + 1;

    A(nspeciesNumberActive + 1, nspeciesNumberActive + 1) =
        -dt*A(nspeciesNumberActive + 1, nspeciesNumberActive + 1) + 1;

    A(nspeciesNumberActive + 2, nspeciesNumberActive + 2) = 1;

    A(nspeciesNumberActive + 3, nspeciesNumberActive + 3) = 1;

    if (this->variableTimeStep())
    {
        A(nspeciesNumberActive + 4, nspeciesNumberActive + 4) = 1;
    }
    ...
    for (label i=0; i<nspeciesNumberActive; i++)
    {
        A(nspeciesNumberActive, i) = 0;
        A(nspeciesNumberActive + 1, i) = 0;
    }
...
template<class CompType, class GThermoType, class SThermoType>
bool Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::retrieve
(
...
)
{
    ...
    if (chemisTree_.size())
    {
        ...
        else if (MRURetrieve_)
        {
            typename SLList
            <
                chemPointISAT<CompType, GThermoType, SThermoType>*
...
template<class CompType, class GThermoType, class SThermoType>
Foam::label Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermoType>::add
(
    const scalarField& phiq,
    const scalarField& Rphiq,
    const label li,
...
)
{
    ...
    if (chemisTree().isFull())
    {
        ...
        if (!cleanAndBalance())
        {
            DynamicList<chemPointISAT<CompType, GThermoType, SThermoType>*> tempList;
            if (maxMRUSize_>0)
            {
                ...
                for ( ; iter != MRUList_.end(); ++iter)
                {
                    tempList.append
                    (
                        new chemPointISAT<CompType, GThermoType, SThermoType>(*iter())
                    );
                }
            }
            chemisTree().clear();
            ...
            chemPointISAT<CompType, GThermoType, SThermoType>* nulPhi = 0;
            forAll(tempList, i)
            {
...
template<class CompType, class GThermoType, class SThermotype>
void
Foam::chemistryGSTabulationMethods::ISATGS<CompType, GThermoType, SThermotype>::writePerformance()
{
    if (this->log())
    {
        nRetrievedFile_()
