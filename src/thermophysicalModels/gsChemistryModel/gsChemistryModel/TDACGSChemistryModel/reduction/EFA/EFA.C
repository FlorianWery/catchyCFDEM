...
Foam::chemistryGSReductionMethods::EFA<CompType, GThermoType, SThermoType>::EFA
(
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemistryGSReductionMethods::EFA<CompType, GThermoType, SThermoType>::reduceMechanism
(
    const scalar p,
    const scalar T,
    const scalarField& c,
...
)
{
    ...
    forAll(this->chemistry_.solidReactions(), i)
    {
        ...
        forAll(R.lhs(),s)
        {
            label curIndex = R.lhs()[s].index + this->nGasSpecie_;
            scalar stoicCoeff = R.lhs()[s].stoichCoeff;
            NCi += sC_[curIndex]*stoicCoeff;
            NHi += sH_[curIndex]*stoicCoeff;
            NOi += sO_[curIndex]*stoicCoeff;
        ...
        forAll(R.glhs(),s)
        {
            label curIndex = R.glhs()[s].index;
            scalar stoicCoeff = R.glhs()[s].stoichCoeff;
            NCi += sC_[curIndex]*stoicCoeff;
            NHi += sH_[curIndex]*stoicCoeff;
            NOi += sO_[curIndex]*stoicCoeff;
            NNi += sN_[curIndex]*stoicCoeff;
        ...
        forAll(R.lhs(), Ai)// compute the element flux
        {
            label A = R.lhs()[Ai].index + this->nGasSpecie_;
            scalar Acoeff = R.lhs()[Ai].stoichCoeff;

            forAll(R.rhs(), Bi)
            {
                label B = R.rhs()[Bi].index + this->nGasSpecie_;
                scalar Bcoeff = R.rhs()[Bi].stoichCoeff;
                // if a pair in the reversed way has not been initialized
                if (rABPos(B, A)==-1)
                {
            ...
            forAll(R.grhs(), Bi)
            {
                label B = R.grhs()[Bi].index;
                scalar Bcoeff = R.grhs()[Bi].stoichCoeff;
                // if a pair in the reversed way has not been initialized
                if (rABPos(B, A)==-1)
                {
                    label otherS = rABPos(A, B);
        ...
        forAll(R.glhs(), Ai)// compute the element flux
        {
            label A = R.glhs()[Ai].index;
            scalar Acoeff = R.glhs()[Ai].stoichCoeff;
        ...
        while(!cumRespected)
        {
            ...
            for (int i=0; i<nbi; i++)
            {
                ...
                if (!this->activeSpecies_[source[idx[startPoint+i]]])
                {
                    ...
                    speciesNumber++;
                    if(source[idx[startPoint+i]] < this->nGasSpecie_)
                    {
                        speciesGasNumber++;
                    }
                    else
                ...
                if (!this->activeSpecies_[sink[idx[startPoint+i]]])
                {
                    ...
                    speciesNumber++;
                    if(sink[idx[startPoint+i]] < this->nGasSpecie_)
                    {
                        speciesGasNumber++;
                    }
                    else
    ...
    this->chemistry_.setNGasSpecie(this->NsSimp_);
    this->chemistry_.setNSolidSpecie(this->NsSimp_);
}
