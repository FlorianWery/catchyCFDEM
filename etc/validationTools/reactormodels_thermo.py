...
class PFR:
    ...
    def solve(self, y0, length, nsteps):
        ...
        states = ct.SolutionArray(self.gas, 1, extra={'z': [0.0], 'tau':[restime], 'vel':[u], 'MW':[self.gas.mean_molecular_weight]})
        ...
        while solver.successful() and solver.t < length:
            ...
            states.append(self.state(solver.y), z=solver.t, tau=restime, vel=u, MW=self.gas.mean_molecular_weight)
...
class surfacePFR_epsB:
    ...
    def solve(self, y0, length, nsteps):
        ...
        restime = 0.0
        u = self.mdot/self.gas.density/self.A
        states = ct.SolutionArray(self.gas, 1, extra={'z': [0.0], 'tau':[restime], 'vel':[u], 'MW':[self.gas.mean_molecular_weight], 'epsB':self.epsB_func(solver.t)})
        surfstates = ct.SolutionArray(self.surf, 1, extra={'z': [0.0]})
        while solver.successful() and solver.t < length:
            solver.integrate(solver.t + dz)
            self.state(solver.y)
            u = self.mdot/self.gas.density/self.A
            restime=restime+dz/u
            states.append(self.state(solver.y), z=solver.t, tau=restime, vel=u, MW=self.gas.mean_molecular_weight, epsB=self.epsB_func(solver.t))
            surfstates.append(self.surf.state, z=solver.t)
