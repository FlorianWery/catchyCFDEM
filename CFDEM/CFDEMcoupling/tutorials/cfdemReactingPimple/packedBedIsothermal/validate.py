...
states.write_csv(output_filename_gas, cols=('extra','T','density','P','Y'))
print("Gas phase results saved to '{0}'".format(output_filename_gas))
surfstates.write_csv(output_filename_surf, cols=('extra','T','X'))
...
latestTime=os.popen("foamListTimes -case CFD | tail -1").read().split('\n')[0]
pfrdata = np.genfromtxt(output_filename_gas, dtype=float, delimiter = ',', names=True)
