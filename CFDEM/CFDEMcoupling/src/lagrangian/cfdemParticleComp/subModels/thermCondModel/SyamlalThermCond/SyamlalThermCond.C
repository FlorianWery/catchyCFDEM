...
tmp<volScalarField> SyamlalThermCond::thermCond() const
{
    ...
    svf.dimensions().reset(dimensionSet(1,1,-3,-1,0,0,0));

    svf = (1-sqrt(max(1-voidfraction_+SMALL,0.0))/ (voidfraction_+SMALL)) * particleCloud_.thermo().kappa()();
...
{
    return thermCond()/(rho_*particleCloud_.thermo().Cp());
}
