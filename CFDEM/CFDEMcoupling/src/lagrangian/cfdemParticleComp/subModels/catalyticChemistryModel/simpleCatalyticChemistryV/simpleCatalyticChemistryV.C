...
#include "error.H"
#include "simpleCatalyticChemistryV.H"
...
defineTypeNameAndDebug(simpleCatalyticChemistryV, 0);

addToRunTimeSelectionTable(catalyticChemistryModel, simpleCatalyticChemistryV, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
simpleCatalyticChemistryV::simpleCatalyticChemistryV
(
...
:
    ...
    surfName_(propsDict_.lookupOrDefault<word>("surfName","surf")),
    gasName_(propsDict_.lookupOrDefault<word>("gasName","gas")),
...
void simpleCatalyticChemistryV::allocateMyArrays() const
{
    double initVal = 0.0;
    ...
    particleCloud_.dataExchangeM().allocateArray(ofPartCoverages_, initVal, nSolidSpecie_);

    if(massTransfer_)
    {
        particleCloud_.dataExchangeM().allocateArray(ofPartGas_, initVal, nGasSpecie_);
...
void simpleCatalyticChemistryV::execute()
{
    forAll(Ygs_, i)
    {
        forAll(Ygs_[i], celli)
    ...
    if (!ini_)
    {
        ...
        {
			for (label i=0; i<nSolidSpecie_; i++) ofPartCoverages_[index][i] = Ysi_[i];
        }
		...
		if(massTransfer_)
		{
			for(int index = 0; index < particleCloud_.numberOfParticles(); ++index)
			{
				for (label i=0; i<nGasSpecie_; i++) ofPartGas_[index][i] = Yi_[i];
			}
		}
		
        //Info << "giving surf vector to LIGGGHTS" << endl;
        //particleCloud_.dataExchangeM().giveData(surfName_, "vector-atom", ofPartCoverages_);
        //Info << "gave surf vector from LIGGGHTS" << endl;
        
        for (label i=0; i<nSolidSpecie_; i++) Info << "ofPartCoverages_[1][i] before getting: " << ofPartCoverages_[1][i] << endl;
        
        Info << "getting surf vector from LIGGGHTS" << endl;
        particleCloud_.dataExchangeM().getData(surfName_, "vector-atom", ofPartCoverages_);
        Info << "got surf vector from LIGGGHTS" << endl;
        
        for (label i=0; i<nSolidSpecie_; i++) Info << "ofPartCoverages_[1][i] after getting: " << ofPartCoverages_[1][i] << endl;
    ...
    else
    {
        ...
        Info << "getting surf vector from LIGGGHTS 2" << endl;
        particleCloud_.dataExchangeM().getData(surfName_, "vector-atom", ofPartCoverages_);
        Info << "got surf vector from LIGGGHTS 2" << endl;         

        if(massTransfer_)
        {
            particleCloud_.dataExchangeM().getData(gasName_, "vector-atom", ofPartGas_);
        };
    }
    ...
    for(int index = 0; index < particleCloud_.numberOfParticles(); ++index)
    {
        ...
        if(cellI >= 0)
        {
            ...
            {
                for (label i=0; i<nGasSpecie_; i++) Yi_[i] = ofPartGas_[index][i];
            };
            
            if(index == 1)
            {
				for (label i=0; i<nSolidSpecie_; i++) Info << "ofPartCoverages_[1][i]: " << ofPartCoverages_[index][i] << endl;
			}
			
			if(index == 2)
            {
				for (label i=0; i<nSolidSpecie_; i++) Info << "ofPartCoverages_[2][i]: " << ofPartCoverages_[index][i] << endl;
			}
			
			if(index == 3)
            {
				for (label i=0; i<nSolidSpecie_; i++) Info << "ofPartCoverages_[3][i]: " << ofPartCoverages_[index][i] << endl;
			}

            for (label i=0; i<nSolidSpecie_; i++) Ysi_[i] = ofPartCoverages_[index][i];

            if(index == 1) Info << "Ysi_ before: " << Ysi_ << endl;
            ...
            for (label i=0; i<nSolidSpecie_; i++) ofPartCoverages_[index][i] = Ysi_[i];
            if(index == 1) Info << "Ysi_ after: " << Ysi_ << endl;
        }
    }
    ...
    forAll(Ys_, i)
    {
        for(int index = 0; index < particleCloud_.numberOfParticles(); ++index)
        {
            partCoverages_[index][0] = ofPartCoverages_[index][i];
        }
...
const volScalarField simpleCatalyticChemistryV::RR(label i) const
{
    return RR_[i];
}

const volScalarField simpleCatalyticChemistryV::Qdot() const
{
    return Qdot_;
}

void simpleCatalyticChemistryV::postFlow()
{
	Info << "Start postFlow chem model" << endl;
    particleCloud_.dataExchangeM().giveData(surfName_, "vector-atom", ofPartCoverages_);

    if(massTransfer_)
    {
            particleCloud_.dataExchangeM().giveData(gasName_, "vector-atom", ofPartGas_);
    };
    ...
    Info << "End postFlow chem model" << endl;
