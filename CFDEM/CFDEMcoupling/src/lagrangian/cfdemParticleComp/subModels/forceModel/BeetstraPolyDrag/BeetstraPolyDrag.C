...
defineTypeNameAndDebug(BeetstraPolyDrag, 0);
...
BeetstraPolyDrag::BeetstraPolyDrag
(
...
void BeetstraPolyDrag::setForce() const
{
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); ++index)
    {
        ...
        if(cellI > -1){
	dsq[cellI] += pow(2*particleCloud_.radius(index),2);
	dtr[cellI] += pow(2*particleCloud_.radius(index),3);
		}
    }
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); ++index)
    {
		...
		dSauter[cellI] = dtr[cellI]/dsq[cellI];

                Us = particleCloud_.velocity(index);
                ds = 2*particleCloud_.radius(index);
                dimlessd = ds/dSauter[cellI];
                dParcel = ds;
                ...
                magUr = mag(Ur);
                Rep=0.0;
                localPhiP = max(SMALL,min(1.-SMALL,1.-voidfraction));

                // calc particle's drag coefficient (i.e., Force per unit slip velocity and per m³ PARTICLE)
                Rep=dSauter[cellI]*voidfraction*magUr/nuf + SMALL;
                F = 10.0*localPhiP/(voidfraction*voidfraction)
		...
		corr = voidfraction*dimlessd + localPhiP*dimlessd*dimlessd + 0.064*voidfraction*pow(dimlessd,3);
                Fi = corr*F;

                dragCoefficient = 3*M_PI*nuf*rho*voidfraction*ds*Fi;
                ...
                if(probeIt_)
                {
                    ...
                    sValues.setSize(sValues.size()+1, dragCoefficient);
