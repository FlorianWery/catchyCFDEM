    ...
    lmp(NULL)
{
    ...
    // Adding additional properties to push/pull list
    if (propsDict_.found("scalarAtomTransferData"))
    {
        List<word> transferFields(propsDict_.lookup("scalarAtomTransferData"));
        forAll(transferFields, i)
        {
            word type = "scalar-atom";
            FixCfdCoupling* fcfd = (FixCfdCoupling*)locate_coupling_fix(lmp);
            fcfd->get_dc()->add_push_property(transferFields[i].c_str(), type.c_str());
            fcfd->get_dc()->add_pull_property(transferFields[i].c_str(), type.c_str());
        }
    }
    if (propsDict_.found("vectorAtomTransferData"))
    {
        List<word> transferFields(propsDict_.lookup("vectorAtomTransferData"));
        forAll(transferFields, i)
        {
            word type = "vector-atom";
            FixCfdCoupling* fcfd = (FixCfdCoupling*)locate_coupling_fix(lmp);
            fcfd->get_dc()->add_push_property(transferFields[i].c_str(), type.c_str());
            fcfd->get_dc()->add_pull_property(transferFields[i].c_str(), type.c_str());
        }
