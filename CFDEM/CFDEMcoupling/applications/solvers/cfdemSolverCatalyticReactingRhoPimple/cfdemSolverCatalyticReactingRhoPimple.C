...
int main(int argc, char *argv[])
{
    ...
    runTime.controlDict().readIfPresent("writeInterval",writeInterval_);
    ...
    while (runTime.run())
    {
        ...
        Switch solveEnergy
        (
            pimple.dict().lookupOrDefault<Switch>("solveEnergy", true)
        );
        ...
        if(std::fmod(runTime.value(),writeInterval_) < runTime.deltaTValue()/2)
	{
	    particleCloud.clockM().evalPar();
	}
