...
ReadDumpToFix::ReadDumpToFix(LAMMPS *lmp) : Pointers(lmp)
{
...
void ReadDumpToFix::command(int narg, char **arg)
{
  if (domain->box_exist == 0)
    error->all(FLERR,"Read_dump_to_fix command before simulation box is defined");

  if (narg < 2) error->all(FLERR,"Illegal read_dump_to_fix command");
...
void ReadDumpToFix::store_files(int nstr, char **str)
{
...
void ReadDumpToFix::file_search(char *infile)
{
  char *ptr;
...
void ReadDumpToFix::setup_reader(int narg, char **arg)
{
  // allocate snapshot field buffer

  memory->create(fields,CHUNK,nfield,"read_dump_to_fix:fields");
...
#undef READER_CLASS
  ...
  else error->all(FLERR,"Invalid dump reader to fix style");
...
bigint ReadDumpToFix::seek(bigint nrequest, int exact)
{
  int ifile,eofflag;
  bigint ntimestep = 0;
...
bigint ReadDumpToFix::next(bigint ncurrent, bigint nlast, int nevery, int nskip)
{
  int ifile,eofflag=0;
  bigint ntimestep = 0;
...
void ReadDumpToFix::header(int fieldinfo)
{
  int triclinic_snap;
...
void ReadDumpToFix::atoms()
{
  ...
  memory->create(uflag,nlocal,"read_dump_to_fix:uflag");
  for (int i = 0; i < nlocal; i++) uflag[i] = 0;
  memory->create(ucflag,CHUNK,"read_dump_to_fix:ucflag");
  memory->create(ucflag_all,CHUNK,"read_dump_to_fix:ucflag");
...
int ReadDumpToFix::fields_and_keywords(int narg, char **arg)
{
...
void ReadDumpToFix::process_atoms(int n)
{
...
void ReadDumpToFix::delete_atoms()
{
...
double ReadDumpToFix::xfield(int i, int j)
{
...
double ReadDumpToFix::yfield(int i, int j)
{
  if (!scaled) return fields[i][j];
...
double ReadDumpToFix::zfield(int i, int j)
{
  if (!scaled) return fields[i][j];
